...
#### What is the Checkdata stored procedure and what does it do?
This script is designed to evaluate various manufacturing tables and report back any inconsistencies it discovers. This would include issues with imported data, issue with rolling up cost, and issue with manufacturing orders and costing.  It will not make any changes to any manufacturing table. It is not version specific and can be run on any versions of Microsoft Dynamics GP.

#### How do you run the Checkdata stored procedure.
...
Once the table has been updated go to BOM Entry 
(Cards | Manufacturing | Bill of Materials pull up the Archived or Configured BOM and either correct the BOM name or delete the BOM.
...
If the BOM is a Manufacturing BOM, go to the BOM Entry (Cards | Manufacturing | Bill of Materials) window and either moved the BOM to an Archived status or deleted.
...
Resolution:
Go to BOM Routing Link (Cards | Manufacturing | Bill of Materials | GoTo | BOM Routing Link), enter the BOM,  delete the linked components and readd them. 
...
Resolution:
Either go to the Standard Cost Changes (Cards | Manufacturing | Inventory| Standard Cost Changes) window or the 
Standard Item Mat Costs (Cards | Manufacturing | Inventory| Standard Item Mat Costs) window to record material costs for the item and then roll up and revalue (Tools |Routines |Manufacturing) so that the item will display the cost. 
...
There is an item with an invalid Item Status (ItemStatus_I) field in the Item Engineering File table (IVR10015). 
Valid numbers for Item Status in this table are 1, 2, 3, 4, 5, and 6. 
...
There is an item that has an invalid Make/Buy Code (MakeBuyCode_I) field in the Item Engineering File table (IVR10015).
The valid values for the Make/Buy Code are 1, 2, and 3. 

Verify that there are no other values in this field in the IVR10015 table. The number should be 1 for a made item, 2 for an either item, or 3 for a bought item.  
...
There is an item with an invalid Order Policy (ORDERPOLICY) field in the Item Engineering File table (IVR10015). 
Valid numbers for this field are 1, 2, or 3.  
...
There is an item marked as floor stock in either BOM Entry (Cards | Manufacturing | Bill of Materials) or in 
Item Engineering window (Cards |Manufacturing | Inventory  |Engineering Data)but not in the other. 
If an item is marked as floorstock it should be marked as floorstock in both location.
...
Resolution:
There are two options for resolution.
Either add the item number in question in the Item Maintenance window (Cards | Inventory | Items), save the item, and then delete the newly added item. 
This will remove the record for the item from both tables. 
...
If the CT00102 is wrong, then update this table via SQL. Use the following SQL scripts to display and then update the records to be changed. 
Replace the XXX with the item number in question. 
